...
describe('methods/eth_gasPrice', function() {
  let requester;
  ...
  it('should return testnet gas price', async function() {
    ...
    expect(json1.result).to.eq("0x3b9aca00");
    ...
    expect(json2.result).to.eq("0x3b9aca00");
    expect(res2).to.have.header('X-CACHE-COUNTER');

    await sendTx();

    // should be still cached after tx
    ...
    expect(json3.result).to.eq("0x3b9aca00");
